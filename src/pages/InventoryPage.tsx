...
export function InventoryPage() {
  ...
  const purchaseItem = async (item: typeof COSMETIC_ITEMS[0]) => {
    if (!user || !profile) return
    setPurchaseError(null)
    setPurchaseSuccess(null)

    if (profile.coins < item.price) {
      setPurchaseError('Not enough coins to purchase this item.')
      return
    }
    ...
    setPurchaseSuccess(`Purchased ${item.item_name}!`)
    refreshProfile()
    fetchItems()
  }
  ...
  const sellItem = async (item: InventoryItem) => {
    ...
    await supabase
      .from('profiles')
      .update({ coins: profile.coins + sellPrice })
      .eq('id', user.id)
  ...
  return (
    <div className="h-full overflow-y-auto pb-4">
      <div className="px-4 py-4">
        ...
        <p className="text-sm text-neutral-500 mb-4">{totalItems} items collected</p>
        ...
        <div className="flex gap-2 mb-4">
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
              activeTab === 'items' ? 'bg-primary-600 text-white' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200'
            }`}
          >
            🎒 Items
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
              activeTab === 'shop' ? 'bg-primary-600 text-white' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200'
            }`}
          >
            🛒 Shop
          </button>
        ...
        {activeTab === 'items' ? (
          loading ? (
            ...
            <div className="card p-8 text-center">
              ...
              <p className="text-neutral-500 font-medium mb-1">Your inventory is empty</p>
              <p className="text-sm text-neutral-400">Complete adventures to find treasures!</p>
            </div>
          ) : (
            <>
              {treasures.length > 0 && (
                    ...
                    {treasures.map((item) => (
                      <div key={item.id} className={`card p-4 border-2 ${RARITY_COLORS[item.rarity].split(' ').find(c => c.startsWith('border')) || 'border-neutral-200'} animate-fade-in`}>
                        <div className="flex items-center justify-between mb-2">
                          ...
                          {item.quantity > 1 && (
                            <span className="badge bg-neutral-100 text-neutral-600">x{item.quantity}</span>
                          )}
                        </div>
                        ...
                        <button
                          ...
                          className="text-xs text-neutral-400 hover:text-error-500 mt-2 font-medium"
                        >
                          Sell for {item.rarity === 'legendary' ? 500 : item.rarity === 'epic' ? 200 : item.rarity === 'rare' ? 80 : 20} 🪙
                        </button>
                      </div>
                    ))}
                  </div>
          ...
          <div>
            ...
            {purchaseSuccess && (
              <div className="mb-3 rounded-xl bg-success-50 border border-success-200 px-4 py-2.5 text-sm text-success-700 animate-fade-in">
                {purchaseSuccess}
              </div>
            )}

            <div className="flex items-center gap-2 mb-4">
              <span className="text-sm text-neutral-500">Your coins:</span>
              <span className="badge bg-accent-50 text-accent-700">🪙 {profile?.coins ?? 0}</span>
            </div>
              ...
              {COSMETIC_ITEMS.map((item) => {
                ...
                return (
                  <div key={item.item_id} className={`card p-4 border-2 ${RARITY_COLORS[item.rarity].split(' ').find(c => c.startsWith('border')) || 'border-neutral-200'}`}>
                    ...
                    <div className="mt-3">
                      {owned ? (
                        <span className="badge bg-success-50 text-success-700 w-full justify-center py-2">Owned</span>
                      ) : (
                        <button
                          onClick={() => purchaseItem(item)}
                          disabled={!canAfford}
                          className="btn w-full py-2 text-sm bg-accent-500 text-white hover:bg-accent-600 disabled:opacity-50 disabled:pointer-events-none"
                        >
                          🪙 {item.price}
                        </button>
                      )}
