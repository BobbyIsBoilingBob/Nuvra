...
export function formatDistance(km: number): string { return km < 1 ? Math.round(km * 1000) + ' m' : km.toFixed(1) + ' km' }
export function formatDuration(min: number): string { if (min < 60) return min + ' min'; const h = Math.floor(min / 60); const m = min % 60; return m > 0 ? h + 'h ' + m + 'm' : h + 'h' }
