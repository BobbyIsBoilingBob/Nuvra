...
import type { NotificationItem, DailyReward, Achievement, AdventureHistoryItem, Adventure } from '@/types/adventure'
...
export async function claimDailyReward(): Promise<{ xp: number; coins: number } | null> {
  if (!supabase) return { xp: 100, coins: 50 }
...
export async function recordAdventureCompletion(data: {
  adventure: Adventure; xpEarned: number; coinsEarned: number; challengesCompleted: number
}): Promise<void> {
  if (!supabase) return
  await supabase.from('adventure_history').insert({
    adventure_name: data.adventure.locationName,
    location_name: data.adventure.locationName,
    duration: data.adventure.durationMin,
    xp_earned: data.xpEarned,
    coins_earned: data.coinsEarned,
    treasures_found: data.challengesCompleted,
