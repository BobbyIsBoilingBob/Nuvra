...
const REWARD_SCALE: Record<Difficulty, { xp: number; coins: number }> = {
  easy: { xp: 15, coins: 10 },
  medium: { xp: 30, coins: 20 },
  hard: { xp: 60, coins: 40 },
  extreme: { xp: 120, coins: 80 },
}

function reward(difficulty: Difficulty, mult = 1): { rewardXp: number; rewardCoins: number } {
  const s = REWARD_SCALE[difficulty]
  return { rewardXp: Math.round(s.xp * mult), rewardCoins: Math.round(s.coins * mult) }
}
...
// ── Additional challenges across categories ─────────────────────────────

const ADDITIONAL: ChallengeTemplate[] = [
  // Landmarks
...
export function challengesByCategory(category: ChallengeCategory): ChallengeTemplate[] {
  return CHALLENGE_LIBRARY.filter((c) => c.category === category)
}

/** Get challenges by difficulty and optional category filter. */
export function challengesForGeneration(
  difficulty: Difficulty,
  categories?: ChallengeCategory[],
): ChallengeTemplate[] {
  let pool = challengesByDifficulty(difficulty)
  // If we don't have enough at the exact difficulty, supplement with adjacent
  if (pool.length < 4) {
    ...
    for (const d of adjacent) {
      pool = pool.concat(challengesByDifficulty(d))
    }
  }
