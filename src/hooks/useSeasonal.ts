...
type Row = { season_id: string; season_name: string; adventures_completed: number; distance_walked: number; target_adventures: number; target_distance: number; reward_claimed: boolean };
const SEASON_ID = 'summer-2026';

function toProgress(r: Row): SeasonalProgress {
  return { seasonId: r.season_id, seasonName: r.season_name, adventuresCompleted: r.adventures_completed, distanceWalked: r.distance_walked, targetAdventures: r.target_adventures, targetDistance: r.target_distance, rewardClaimed: r.reward_claimed };
}
...
export function useSeasonal() {
  ...
  const recordProgress = useCallback(async (distanceM: number) => {
    if (!progress) return;
    const next: SeasonalProgress = { ...progress, adventuresCompleted: progress.adventuresCompleted + 1, distanceWalked: progress.distanceWalked + distanceM };
    setProgress(next);
    const { error } = await supabase.from('seasonal_progress').update({ adventures_completed: next.adventuresCompleted, distance_walked: next.distanceWalked, updated_at: new Date().toISOString() }).eq('season_id', SEASON_ID);
