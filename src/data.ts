export type Quest = {
  ...
  description: string;
  xp: number;
  type: 'location' | 'distance' | 'discovery' | 'social';
  target?: number;
};
...
export const ADVENTURES: Adventure[] = [
  ...
  },
  {
    id: 'nature-trail',
    name: 'Nature Trail',
    description: 'Connect with nature on a scenic walk',
    theme: 'Nature',
    difficulty: 'medium',
    estimatedMinutes: 45,
    totalXp: 400,
  ...
  },
  {
    id: 'treasure-hunt',
    name: 'Treasure Hunt',
    description: 'Follow clues to hidden treasure',
    theme: 'Mystery',
    difficulty: 'hard',
    estimatedMinutes: 60,
    totalXp: 600,
  ...
  },
  {
    id: 'night-prowl',
    name: 'Night Prowl',
    description: 'A mysterious adventure after dark',
    theme: 'Night',
    difficulty: 'epic',
    estimatedMinutes: 50,
    totalXp: 750,
...
export function getComboTier(combo: number) {
  if (combo >= 10) return { name: 'Legendary', color: '#f59e0b', multiplier: 3 };
  if (combo >= 5) return { name: 'Epic', color: '#a78bfa', multiplier: 2 };
  if (combo >= 3) return { name: 'Great', color: '#3b82f6', multiplier: 1.5 };
  return { name: 'Normal', color: '#64748b', multiplier: 1 };
}

export const DAILY_REWARDS = Array.from({ length: 7 }, (_, i) => ({
  day: i + 1,
  coins: 100 + i * 50,
  gems: i === 6 ? 5 : 0,
}));
...
export function generateAdventure(): Adventure {
  ...
  return {
    id: `gen-${Date.now()}`,
    name: `${theme} Expedition`,
    description: `A procedurally generated ${theme.toLowerCase()} adventure`,
    theme,
    difficulty: 'medium',
    estimatedMinutes: 30 + Math.floor(Math.random() * 30),
    totalXp: 300 + Math.floor(Math.random() * 300),
    emoji: emojis[theme],
    color: colors[theme],
    quests: [
