...
export const DIFFICULTY_COLORS: Record<Difficulty, string> = { relaxed: '#22c55e', easy: '#4de8ff', medium: '#fbbf24', hard: '#fb923c', extreme: '#ef4444' };
...
function genRoute(seed: number, complexity: number): { x: number; y: number }[] {
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    points.push({
      x: Math.round(50 + Math.cos(angle) * radius * (1 - (i / numPoints) * 0.3)),
      y: Math.round(50 + Math.sin(angle) * radius * (1 - (i / numPoints) * 0.3)),
    });
  }
  return points;
}
...
export function generateAdventure(seed: number, type: AdventureType, difficulty: Difficulty): Adventure {
  const adjectives = ['Mystic', 'Hidden', 'Golden', 'Forgotten', 'Ancient', 'Secret', 'Crystal', 'Emerald'];
  const nouns = ['Trail', 'Path', 'Way', 'Loop', 'Circuit', 'Journey', 'Quest', 'Expedition'];
  ...
  return {
    id: `gen-${seed}-${Date.now()}`, title: `${adj} ${noun}`, description: `Explore a ${adj.toLowerCase()} route through the city. Discover hidden landmarks and collect treasures along the way.`,
    type, difficulty, distance: distMap[difficulty], duration: Math.round(distMap[difficulty] * 12),
...
export type Quest = { id: string; title: string; description: string; metric: string; target: number; reward: { xp: number; coins: number }; icon: string; category: 'daily' | 'weekly' | 'story' };
...
export type Challenge = { id: string; title: string; description: string; difficulty: Difficulty; xpReward: number; coinReward: number; icon: string };
