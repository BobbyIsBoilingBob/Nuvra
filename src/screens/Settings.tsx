import { Icon, GlassCard, Button } from '../components/ui';
import { AdventureBg } from '../components/AdventureBg';
import { TopBar } from '../components/BottomNav';
import { useStore } from '../store';

export function Settings(): React.ReactElement {
  const { profile, setProfile, resetProgress, setScreen } = useStore();
    ...
    <div className="relative min-h-screen w-full overflow-hidden pb-24">
      ...
      <div className="relative z-10">
        ...
        <div className="px-4 max-w-md mx-auto flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setProfile({ username: e.target.value.slice(0, 20) })}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm font-bold outline-none focus:border-nova-400/50"
                  maxLength={20}
                />
              </div>
            </div>
          ...
          {/* Game info */}
          <GlassCard className="p-4">
            <h3 className="text-xs font-black uppercase tracking-wider text-white/60 mb-3">About</h3>
            <div className="flex flex-col gap-2 text-xs text-white/50">
              <div className="flex justify-between"><span>Version</span><span className="text-white/70">10.0.0</span></div>
