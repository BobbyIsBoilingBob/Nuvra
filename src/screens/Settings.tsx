...
export default function Settings() {
  const { profile, signOut, deleteAccount, isGuest, exitGuest } = useAuth(); const { resetProgress } = useStore(); const { settings, update } = useSettings(); const [showDelete, setShowDelete] = useState(false);
  if (isGuest || !profile) return <Screen><h1 className="font-display text-2xl font-bold text-white mb-4">Settings</h1><Card className="p-6 text-center"><p className="text-ink-400 text-sm mb-4">Sign in to access account settings and preferences.</p><Button className="w-full" onClick={() => exitGuest()}>Sign In or Create Account</Button></Card></Screen>;
  const Toggle = ({ icon: Icon, label, value, onChange }: { icon: any; label: string; value: boolean; onChange: (v: boolean) => void }) => <button onClick={() => onChange(!value)} className="w-full flex items-center gap-3 p-3"><Icon size={20} color="#94a3b8" /><span className="flex-1 text-left text-white">{label}</span><div className={`w-11 h-6 rounded-full transition-colors ${value ? 'bg-zeviqo-500' : 'bg-ink-600'}`}><div className={`w-5 h-5 rounded-full bg-white transition-transform mt-0.5 ${value ? 'translate-x-5' : 'translate-x-0.5'}`} /></div></button>;
  return (
    <Screen>
