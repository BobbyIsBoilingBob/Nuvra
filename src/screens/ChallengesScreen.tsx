...
const categoryIconMap: Record<ChallengeCategory, typeof Target> = {
  trivia: Brain, photo: Camera, fitness: Footprints, compass: Compass,
  riddle: Brain, speed: Zap, exploration: Compass, puzzle: Brain,
}
        ...
        <div className="space-y-4">
          <div className="flex gap-2">
            {[
              ...
              <button key={f.id} onClick={() => setFilter(f.id)} className={`chip flex-1 text-center ${filter === f.id ? 'chip-active' : 'chip-inactive'}`}>{f.label}</button>
            ))}
          </div>
                ...
                return (
                  <div key={c.id} className={`card-premium p-4 stagger ${c.completed ? 'opacity-60' : ''}`} style={{ animationDelay: `${i * 40}ms` }}>
                    <div className="flex items-start gap-3">
                      <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${c.completed ? 'bg-success-100' : 'bg-gradient-to-br from-brand-500 to-brand-600'}`}>
                        {c.completed ? <Check size={20} className="text-success-600" /> : <Icon size={20} className="text-white" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-ink-900">{c.title}</p>
                        <p className="text-xs text-ink-400 capitalize mt-0.5">{c.category} challenge</p>
                        <div className="flex items-center gap-3 mt-2">
                          <span className="flex items-center gap-1 text-xs font-semibold text-brand-600"><Star size={12} /> +{c.xp} XP</span>
                          <span className="flex items-center gap-1 text-xs font-semibold text-accent-600"><Coins size={12} /> +{c.coins}</span>
                          <span className={`flex items-center gap-1 text-xs ${c.completed ? 'text-success-600' : 'text-ink-400'}`}><Clock size={12} /> {c.timeLeft}</span>
                        </div>
                      </div>
                      {!c.completed && (
                        <button onClick={() => { setChallenges(prev => prev.map(x => x.id === c.id ? { ...x, completed: true, timeLeft: 'Completed' } : x)); onNavigate('generator') }} className="px-3 py-2 bg-brand-50 border border-brand-200 text-brand-600 rounded-xl text-xs font-bold btn-press hover:bg-brand-100 transition flex items-center gap-1">
