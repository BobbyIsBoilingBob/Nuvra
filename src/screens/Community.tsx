...
import { Card, Screen, Button, EmptyState } from '../components/ui';
import { Bell, Users, UserPlus, Heart, MessageCircle, PartyPopper } from 'lucide-react';
        ...
        {items.map(item => (
          <Card key={item.id} className="p-4 cursor-pointer hover:border-zeviqo-500/30 transition-all" onClick={() => setScreen(item.id as never)}>
            <div className="flex items-center gap-3">
              ...
              {item.badge ? (
                <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-red-500/20 text-red-400">{item.badge}</span>
              ) : null}
            </div>
          </Card>
        ))}
