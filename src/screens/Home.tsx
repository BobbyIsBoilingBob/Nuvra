      ...
      <div className="relative z-10">
        ...
        <div className="px-4 max-w-md mx-auto flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <StatChip icon="Footprints" label="Distance" value={`${(stats.totalDistance/1000).toFixed(1)} km`} color="text-zeviqo-300" />
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => setScreen('adventures')} className="text-xs font-bold text-zeviqo-300">See all</button>
            </div>
            <GlassCard className="p-4 cursor-pointer hover:bg-white/[0.1] transition-all" onClick={() => { setScreen('adventure-detail'); }}>
              <div className="flex items-center gap-3">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-plasma-400 to-zeviqo-500 flex items-center justify-center text-3xl">{featuredAdventure.emoji}</div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <Pill icon="Zap" accent="text-zeviqo-300 border-zeviqo-500/30">+{featuredAdventure.xpReward} XP</Pill>
          ...
          <div className="grid grid-cols-4 gap-2">
            <button onClick={() => setScreen('quests')} className="glass rounded-2xl p-3 flex flex-col items-center gap-1.5 hover:bg-white/10 transition-all active:scale-95">
              <Icon name="ScrollText" size={20} className="text-zeviqo-300" /><span className="text-[10px] font-bold text-white/60">Quests</span>
            </button>
          ...
          {recentHistory.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => setScreen('history')} className="text-xs font-bold text-zeviqo-300">View all</button>
              </div>
              <div className="flex flex-col gap-2">
                {recentHistory.map(h => (
                  <GlassCard key={h.id} className="p-3 flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-plasma-400/30 to-zeviqo-500/30 flex items-center justify-center text-xl">{h.emoji}</div>
                    <div className="flex-1 min-w-0">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => setScreen('quests')} className="text-xs font-bold text-zeviqo-300">View all</button>
            </div>
            <div className="flex flex-col gap-2">
              {activeQuests.map(q => {
                let progress = 0;
                ...
                return (
                  <GlassCard key={q.id} className="p-3">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl glass flex items-center justify-center flex-shrink-0"><Icon name={q.icon} size={16} className="text-zeviqo-300" /></div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="w-full h-1.5 rounded-full bg-white/10 mt-1.5 overflow-hidden">
                          <div className="h-full rounded-full bg-gradient-to-r from-zeviqo-400 to-cyan-300 transition-all duration-500" style={{ width: `${pct}%` }} />
                        </div>
                      </div>
