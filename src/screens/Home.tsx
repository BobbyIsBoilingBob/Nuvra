...
import { Compass, Trophy, Target, Calendar, Users, Award, History, Package, User as UserIcon, Users as Community, Sparkles, LogIn, UserPlus, Map } from 'lucide-react';
    ...
    <div className="px-4 py-6">
      ...
      {isGuest && (
        <Card className="mb-6 bg-brand-50 border-brand-100">
          <p className="text-sm text-brand-800">You're exploring as a guest. Sign in to save progress, earn rewards, and join parties.</p>
      ...
      <div className="grid grid-cols-2 gap-3">
        {TILES.map(({ screen, label, icon: Icon, desc }) => (
          <Card key={label} onClick={() => navigate(screen)} className="flex flex-col gap-2">
            ...
            <div><p className="font-semibold text-ink-900">{label}</p><p className="text-xs text-ink-500">{desc}</p></div>
          </Card>
        ))}
      </div>
      {isGuest && (
        <div className="mt-6 flex gap-2">
          <Button variant="ghost" fullWidth onClick={() => navigateToAuth('home')}><LogIn size={16} className="inline mr-1" /> Sign In</Button>
          <Button variant="ghost" fullWidth onClick={() => navigateToAuth('home')}><UserPlus size={16} className="inline mr-1" /> Create Account</Button>
        </div>
      )}
    </div>
  );
