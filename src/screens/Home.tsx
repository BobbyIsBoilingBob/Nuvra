...
import { getAdventures, getLevelProgress, DIFFICULTY_LABELS, ADVENTURE_TYPES } from '../data';

export function Home() {
  const { profile } = useAuth();
  const { setScreen, setSelectedAdventure, history, dailyRewardStreak, lastDailyRewardDate } = useStore();
      ...
      <div className="relative z-10 px-4 pt-6 space-y-5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setScreen('community')} className="w-10 h-10 rounded-xl glass flex items-center justify-center active:scale-90 transition-transform relative">
