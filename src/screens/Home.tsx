...
export default function Home() {
  ...
  const combo = getComboTier(profile.walking_streak);
    ...
    <Screen>
      ...
      <Card className="p-4 mb-4">
        <div className="flex items-center gap-2 mb-3">
          <Map size={18} color="#00c4ff" />
          <h3 className="font-semibold text-white">Featured Adventures</h3>
        </div>
          ...
          {adventures.slice(0, 2).map(a => (
            <button key={a.id} onClick={() => setScreen('adventures')}
              ...
              <div className="flex-1">
                ...
                <p className="text-ink-400 text-xs">{a.distance} km · {a.duration} min</p>
              </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Button variant="primary" onClick={() => setScreen('adventures')} className="flex items-center justify-center gap-2">
          <Map size={18} /> Start Adventure
        </Button>
        <Button variant="gold" onClick={() => setScreen('daily-rewards')} className="flex items-center justify-center gap-2">
