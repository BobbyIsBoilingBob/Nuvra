...
export function Profile() {
  ...
  const stats = [
    { icon: 'Route', label: 'Total Distance', value: `${profile.totalDistance.toFixed(2)} km`, color: 'text-zeviqo-300' },
    { icon: 'Footprints', label: 'Total Steps', value: profile.totalSteps.toLocaleString(), color: 'text-cyan-300' },
    { icon: 'Compass', label: 'Adventures', value: profile.totalAdventures, color: 'text-ember-300' },
    { icon: 'Trophy', label: 'Challenges', value: profile.totalChallenges, color: 'text-gold-300' },
    { icon: 'Gem', label: 'Treasures', value: profile.totalTreasures, color: 'text-plasma-300' },
    { icon: 'Flame', label: 'Walking Streak', value: `${profile.walkingStreak} days`, color: 'text-ember-300' },
    ...
    { icon: 'Gem', label: 'Gems', value: profile.gems, color: 'text-plasma-300' }
  ];
      ...
      <div className="relative z-10">
        ...
        <div className="px-4 max-w-md mx-auto flex flex-col gap-4 pt-4">
          <GlassCard className="p-5 flex flex-col items-center text-center">
            <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-zeviqo-400/20 to-plasma-500/20 flex items-center justify-center text-5xl mb-3">{profile.avatarEmoji}</div>
            ...
            <div className="flex gap-1.5 mt-2">
              ...
              <Pill icon="Calendar" accent="text-white/60 border-white/10">Since {new Date(profile.createdAt).toLocaleDateString()}</Pill>
            </div>
