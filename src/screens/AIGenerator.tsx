...
import { generateAdventure, ADVENTURE_TYPES, type AdventureType, type Difficulty } from '../data';
...
export default function AIGenerator() {
  ...
  const handlePlay = () => {
    if (generated) { setSelectedAdventureObj(generated); setScreen('adventure-preview'); }
  };

  return (
    <Screen>
      <h1 className="font-display text-2xl font-bold text-white mb-2 flex items-center gap-2">
        <Sparkles size={24} color="#a78bfa" /> AI Adventure Generator
      </h1>
        ...
        <div className="grid grid-cols-2 gap-2">
          {ADVENTURE_TYPES.map(t => (
            <button key={t.type} onClick={() => setType(t.type)}
              className={`p-3 rounded-xl text-sm font-medium transition-all ${type === t.type ? 'bg-zeviqo-500 text-ink-950' : 'bg-ink-700/50 text-ink-400'}`}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {(Object.keys(DIFFICULTY_LABELS) as Difficulty[]).map(d => (
            <button key={d} onClick={() => setDifficulty(d)}
              className={`px-3 py-1.5 rounded-full text-sm font-medium ${difficulty === d ? 'text-ink-950' : 'bg-ink-700/50 text-ink-400'}`}
              style={difficulty === d ? { background: DIFFICULTY_COLORS[d] } : {}}>
              {DIFFICULTY_LABELS[d]}
            </button>
          ))}
        </div>
      ...
      <Button onClick={handleGenerate} className="w-full mb-4 flex items-center justify-center gap-2">
      ...
      {generated && (
        <Card className="p-4 mb-4 border-zeviqo-500/30">
          <div className="flex items-start gap-3 mb-3">
            <div className="text-3xl">{generated.emoji}</div>
            <div className="flex-1">
              <h3 className="font-semibold text-white">{generated.title}</h3>
              <p className="text-ink-400 text-sm">{generated.description}</p>
            </div>
          </div>
          ...
          <Button onClick={handlePlay} className="w-full mt-3 flex items-center justify-center gap-2">
            <Play size={16} /> Preview Adventure
          </Button>
        </Card>
      )}
