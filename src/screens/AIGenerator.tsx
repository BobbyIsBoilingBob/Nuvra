...
export default function AIGenerator() {
  ...
  if (isGuest) {
    return (
      <div className="pb-24"><Header title="AI Adventure Generator" />
  ...
  const generate = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        const res = await createAdventure(adv);
    ...
    <div className="pb-24"><Header title="AI Adventure Generator" />
      <div className="px-4 py-4 max-w-lg mx-auto space-y-4">
        <Card className="p-5">
          ...
          <p className="text-ink-400 text-sm mb-3">Describe the kind of walking adventure you want and the AI will create a route with checkpoints and rewards.</p>
          <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="A relaxed riverside walk with two checkpoints…"
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {PROMPT_IDEAS.map((idea) => (
              <button key={idea} onClick={() => setPrompt(idea)} className="px-2 py-1 rounded-lg text-xs bg-ink-700 text-ink-300 hover:bg-ink-600 transition-colors">{idea}</button>
            ))}
          </div>
          <Button className="w-full mt-3" onClick={generate} disabled={generating}>{generating ? 'Generating…' : <><Wand2 size={18} /> Generate Adventure</>}</Button>
        ...
        {result && (
          <Card className="p-5 animate-slide-up">
            <div className="flex items-center gap-2 mb-3"><Check size={20} className="text-success-400" /><h3 className="font-display font-bold text-white">Adventure generated!</h3></div>
            ...
            <div className="mt-3"><p className="text-ink-400 text-xs font-medium mb-1">Quests:</p>
              <ul className="space-y-1">{result.quests.map((q, i) => (<li key={q.id} className="text-ink-300 text-sm flex items-center gap-2"><span className="h-5 w-5 rounded-full bg-ink-700 flex items-center justify-center text-[10px] font-bold text-ink-300">{i + 1}</span>{q.title}</li>))}</ul>
            </div>
            <Button className="w-full mt-4" onClick={startAdventure}><Navigation size={18} /> Start This Adventure</Button>
          </Card>
        )}
      </div>
    </div>
