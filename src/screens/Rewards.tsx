...
import { Coins, Gem, Calendar } from 'lucide-react';

export default function Rewards() {
  const { dailyRewardStreak, lastDailyRewardDay, setScreen } = useStore();
  return (
    <Screen>
      <h1 className="font-display text-2xl font-bold text-white mb-4 flex items-center gap-2"><Calendar size={24} color="#fbbf24" /> Rewards</h1>
      ...
      <div className="grid grid-cols-2 gap-3">{DAILY_REWARDS.map(reward => {
        ...
        return (
          <Card key={reward.day} className={`p-4 ${isClaimed ? 'opacity-60' : ''}`}>
            <div className="flex items-center justify-between mb-2"><span className="text-ink-400 text-xs">Day {reward.day}</span></div>
