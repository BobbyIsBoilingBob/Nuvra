...
export default function PartyScreen({ onBack, onToast }: Props) {
  ...
  return (
    <ScreenShell title="Party" icon={<PartyPopper size={18} />} onBack={onBack}
      headerRight={
        <button onClick={() => setShowCreate(s => !s)} className="text-brand-400 hover:text-brand-300 transition active:scale-90 p-1">
          <Plus size={20} />
        </button>
      }
    >
      {showCreate && (
        <div className="bg-ink-900 border border-ink-800 rounded-xl p-3 mb-4 space-y-3 animate-slide-up">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-ink-200">Create Party</h3>
            <button onClick={() => setShowCreate(false)} className="text-ink-500 hover:text-ink-300 transition"><X size={16} /></button>
          </div>
          <input
            type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Party name"
      ...
      {loading ? <LoadingSpinner /> : parties.length === 0 ? (
        ...
        <div className="space-y-2">
          {parties.map(p => (
            <div key={p.id} className="bg-ink-900 border border-ink-800 rounded-xl p-3.5 animate-fade-in">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-ink-100 truncate">{p.name}</p>
