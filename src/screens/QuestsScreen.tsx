...
export default function QuestsScreen({ onNavigate }: Props) {
  const daily = mockQuests.filter(q => q.type === 'daily'), weekly = mockQuests.filter(q => q.type === 'weekly'), other = mockQuests.filter(q => q.type !== 'daily' && q.type !== 'weekly')
  const renderQuest = (q: Quest, i: number) => { const pct = Math.min((q.progress / q.target) * 100, 100); const done = q.progress >= q.target; return (
    <div key={q.id} className="card-premium p-4 stagger" style={{ animationDelay: i * 40 + 'ms' }}>
      <div className="flex items-start gap-3 mb-3">{done ? <CheckCircle2 size={20} className="text-success-500 flex-shrink-0 mt-0.5" /> : <Circle size={20} className="text-ink-300 flex-shrink-0 mt-0.5" />}<div className="flex-1 min-w-0"><p className="text-sm font-bold text-ink-900">{q.title}</p><p className="text-xs text-ink-400 mt-0.5">{q.description}</p></div><div className="flex items-center gap-2 flex-shrink-0"><span className="flex items-center gap-1 text-xs font-semibold text-brand-600"><Star size={12} /> {q.xp}</span><span className="flex items-center gap-1 text-xs font-semibold text-accent-600"><Coins size={12} /> {q.coins}</span></div></div>
      <div className="flex items-center gap-3"><div className="flex-1 h-2.5 bg-surface-200 rounded-full overflow-hidden"><div className={'h-full rounded-full transition-all duration-500 ' + (done ? 'bg-success-500' : 'bg-gradient-to-r from-brand-500 to-accent-500')} style={{ width: pct + '%' }} /></div><span className="text-xs font-bold text-ink-500">{q.progress}/{q.target}</span>{q.expires_at && !done && <span className="text-xs text-ink-400 flex items-center gap-1"><Clock size={10} /> {new Date(q.expires_at).toLocaleDateString('en', { weekday: 'short' })}</span>}</div>
    </div>
  ) }
  return (
    <>
