  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden pb-24">
      ...
      <div className="relative z-10">
        <TopBar title="Adventure History" showBack />
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => toggleFavorite(entry.id)} aria-label="Toggle favorite" className="flex-shrink-0">
                          <Icon name="Heart" size={16} className={entry.isFavorite ? 'text-rose-400 fill-current' : 'text-white/30'} />
                        </button>
                      </div>
                      ...
                      {entry.challengesCompleted.length > 0 && (
                        <div className="flex items-center gap-1 mt-1.5">
                          <Icon name="Trophy" size={12} className="text-gold-300" />
                          <span className="text-[10px] text-white/40">{entry.challengesCompleted.length} challenges completed</span>
                        </div>
                      )}
        ...
        {detailEntry && (
          <div className="flex flex-col gap-4">
            ...
            {detailEntry.challengesCompleted.length > 0 && (
              <div>
                <div className="text-xs font-bold text-white/60 mb-1">Challenges Completed</div>
                <div className="flex flex-wrap gap-2">
                  {detailEntry.challengesCompleted.map((c, i) => <Pill key={i} icon="Trophy" accent="text-gold-300 border-gold-500/30">{c}</Pill>)}
                </div>
              </div>
            )}
            ...
            <div className="flex gap-2">
              <Button variant="secondary" size="sm" fullWidth icon="Heart" onClick={() => { toggleFavorite(detailEntry.id); setDetailEntry({...detailEntry, isFavorite: !detailEntry.isFavorite}); }}>
