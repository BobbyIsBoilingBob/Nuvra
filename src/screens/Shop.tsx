...
export default function Shop() {
    ...
    return (
      <Screen>
        ...
        <Card className="p-6 text-center">
          <ShoppingBag size={32} color="#fbbf24" className="mx-auto mb-3" />
          <h2 className="font-display text-lg font-bold text-white mb-2">Sign in to shop</h2>
          <p className="text-ink-400 text-sm mb-4">Create an account to buy and own cosmetics.</p>
  ...
  const categories: { id: InventoryCategory | 'seasonal'; label: string }[] = [
    { id: 'trails', label: 'Trails' }, { id: 'pets', label: 'Pets' }, { id: 'themes', label: 'Themes' },
    { id: 'stickers', label: 'Stickers' }, { id: 'badges', label: 'Badges' }, { id: 'seasonal', label: 'Seasonal' },
  ];
  ...
  return (
    <Screen>
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto no-select pb-1">
        {categories.map((c) => (
          <button key={c.id} onClick={() => setCategory(c.id)} className={`px-3 py-1.5 rounded-full text-sm font-medium whitespace-nowrap ${category === c.id ? 'bg-zeviqo-500 text-ink-950' : 'bg-ink-700/50 text-ink-400'}`}>{c.label}</button>
        ))}
      </div>
      {items.length === 0 ? <EmptyState icon={ShoppingBag} title="No items available" /> : (
          ...
          {items.map((item) => {
            ...
            return (
              <Card key={item.id} className="p-3" style={{ borderColor: `${RARITY_COLORS[item.rarity]}33` }}>
                ...
                <div className="flex items-center justify-between mb-2"><Badge color={RARITY_COLORS[item.rarity]}>{RARITY_LABELS[item.rarity]}</Badge></div>
                {owned ? (
                  <Button size="sm" variant="secondary" className="w-full" onClick={() => equipItem(item.category, item.id)}><Check size={14} className="inline" /> Equip</Button>
                ) : (
                  <Button size="sm" variant={item.currency === 'gems' ? 'gold' : 'primary'} className="w-full" disabled={!canAfford} onClick={() => handleBuy(item)}>
