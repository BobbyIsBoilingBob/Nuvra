...
import { Icon, GlassCard, Pill, Button, EmptyState } from '../components/ui';
...
export function Shop(): React.ReactElement {
  const { profile, spendCoins, spendGems } = useStore();
    ...
    <div className="relative min-h-screen w-full overflow-hidden pb-24">
      ...
      <div className="relative z-10">
        ...
        <div className="px-4 max-w-md mx-auto flex flex-col gap-4">
          {error && (
            <div className="glass rounded-xl px-4 py-2.5 text-xs text-rose-300 font-bold text-center">{error}</div>
          )}
            ...
            {SHOP_ITEMS.map((item) => {
              ...
              return (
                <GlassCard key={item.id} className={`p-4 flex flex-col items-center gap-2 ${isPurchased ? 'opacity-50' : ''}`}>
                  <div className={`w-14 h-14 rounded-2xl glass flex items-center justify-center text-3xl ${rarity.glow}`}>
                    {item.emoji}
                  </div>
                  ...
                  {isPurchased ? (
                    <div className="flex items-center gap-1 text-xs font-bold text-nova-300">
                      <Icon name="CheckCircle" size={14} /> Owned
                    </div>
                  ) : (
                    <Button size="sm" variant="secondary" icon="Coins" onClick={() => handleBuy(item.id, item.price)} disabled={profile.coins < item.price}>
                      {item.price}
                    </Button>
                  )}
                </GlassCard>
