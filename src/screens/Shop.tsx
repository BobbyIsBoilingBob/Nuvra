...
import { Coins, Gem, Check, ShoppingBag, Backpack, LogIn } from 'lucide-react';

export default function Shop() {
  const { profile, updateProfile, isGuest, exitGuest } = useAuth(); const { ownedItems, buyItem, equipItem, setScreen } = useStore(); const [category, setCategory] = useState<InventoryCategory | 'seasonal'>('trails');
  if (isGuest || !profile) return <Screen><h1 className="font-display text-2xl font-bold text-white mb-4">Shop</h1><Card className="p-6 text-center"><ShoppingBag size={32} color="#fbbf24" className="mx-auto mb-3" /><h2 className="font-display text-lg font-bold text-white mb-2">Sign in to shop</h2><p className="text-ink-400 text-sm mb-4">Create an account to buy and own cosmetics.</p><Button className="w-full flex items-center justify-center gap-2" onClick={() => exitGuest()}><LogIn size={18} /> Sign In or Create Account</Button></Card></Screen>;
  ...
  const handleBuy = async (item: CosmeticItem) => { if (!profile || ownedItems.includes(item.id)) return; if (item.currency === 'coins' && profile.coins < item.price) return; if (item.currency === 'gems' && profile.gems < item.price) return; const success = buyItem(item.id); if (success) await updateProfile(item.currency === 'coins' ? { coins: profile.coins - item.price } : { gems: profile.gems - item.price }); };
  return (
    <Screen>
      <h1 className="font-display text-2xl font-bold text-white mb-4">Shop</h1>
      <div className="flex gap-2 mb-4"><Badge color="#fbbf24"><Coins size={12} className="inline" /> {profile.coins.toLocaleString()}</Badge><Badge color="#a78bfa"><Gem size={12} className="inline" /> {profile.gems}</Badge></div>
