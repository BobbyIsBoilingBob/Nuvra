  ...
  return (
    <Screen>
      ...
      <div className="flex gap-2 mb-4">
        <Badge color="#fbbf24"><Coins size={12} className="inline" /> {profile?.coins.toLocaleString() ?? 0}</Badge>
        <Badge color="#a78bfa"><Gem size={12} className="inline" /> {profile?.gems ?? 0}</Badge>
      </div>
      ...
      : <div className="grid grid-cols-2 gap-3">{items.map(item => {
        ...
        return (
          <Card key={item.id} className="p-3" style={{ borderColor: `${RARITY_COLORS[item.rarity]}33` }}>
            ...
            {owned ? <Button size="sm" variant="secondary" className="w-full" onClick={() => equipItem(item.category, item.id)}><Check size={14} className="inline" /> Equip</Button>
