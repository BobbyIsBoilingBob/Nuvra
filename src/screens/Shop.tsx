...
export default function Shop() {
  ...
  const categories: { id: InventoryCategory | 'seasonal'; label: string }[] = [
    { id: 'trails', label: 'Trails' }, { id: 'pets', label: 'Pets' },
    { id: 'themes', label: 'Themes' }, { id: 'stickers', label: 'Stickers' },
    { id: 'badges', label: 'Badges' }, { id: 'seasonal', label: 'Seasonal' },
  ];
  ...
  const handleBuy = async (item: CosmeticItem) => {
    ...
    if (success) {
      await updateProfile(item.currency === 'coins'
  ...
  return (
    <Screen>
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto no-select pb-1">
        {categories.map(c => (
          <button key={c.id} onClick={() => setCategory(c.id)}
            className={`px-3 py-1.5 rounded-full text-sm font-medium whitespace-nowrap ${category === c.id ? 'bg-zeviqo-500 text-ink-950' : 'bg-ink-700/50 text-ink-400'}`}>
            {c.label}
          </button>
        ))}
      </div>
      ...
      {items.length === 0 ? (
        <EmptyState icon={ShoppingBag} title="No items available" />
      ) : (
