      ...
      {loading ? <LoadingSpinner label="Loading rankings..." /> : players.length === 0 ? (
        ...
        <div className="space-y-2">
          {players.map((p, i) => {
            ...
            return (
              <div key={p.id} className={`flex items-center gap-3 rounded-xl p-3 border animate-fade-in ${isMe ? 'bg-brand-500/15 border-brand-500/40' : 'bg-ink-900 border-ink-800'}`}>
                ...
                <div className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: p.avatar_color || '#10b981' }}>
                  <Star size={16} className="text-white" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-ink-100 truncate">{p.username}{isMe && ' (You)'}</p>
                  <p className="text-xs text-ink-500">Level {p.level}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-brand-400 tabular-nums">{p.xp}</p>
