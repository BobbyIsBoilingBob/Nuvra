...
export function DailyRewards(): React.ReactElement {
  const { dailyReward, claimDailyReward, setScreen } = useStore();
        ...
        <div className="px-4 max-w-md mx-auto flex flex-col gap-4">
          ...
          <div className="grid grid-cols-4 gap-2">
            {DAILY_LOGIN_REWARDS.map((reward, i) => {
              ...
              const isLocked = dayNum > claimableDay;
              ...
              return (
                <GlassCard
                  ...
                  } ${isClaimable ? 'ring-2 ring-nova-400 animate-pulse' : ''} ${
                    isWeekBonus ? 'col-span-4 flex-row justify-around py-4' : ''
                  }`}
                >
                  {isClaimed && (
                  ...
                  <div className={`text-[10px] font-bold uppercase tracking-wider ${isClaimable ? 'text-nova-300' : 'text-white/40'}`}>
                    {reward.label}
                  </div>
                  {isWeekBonus ? (
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-sm font-black text-white">+{reward.coins}</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <Icon name="Gem" size={16} className="text-plasma-400" />
                          <span className="text-sm font-black text-white">+{reward.gems}</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <Icon name="Zap" size={16} className="text-nova-300" />
                          <span className="text-sm font-black text-white">+{reward.xp}</span>
                        </div>
                      </div>
                    </div>
                  ) : (
                    <>
                      ...
                      <div className="flex items-center gap-1">
                        <Icon name="Coins" size={10} className="text-gold-300" />
                        <span className="text-[10px] font-bold text-white">{reward.coins}</span>
                      </div>
                      {reward.gems > 0 && (
                        <div className="flex items-center gap-1">
                          <Icon name="Gem" size={10} className="text-plasma-400" />
                          <span className="text-[10px] font-bold text-white">{reward.gems}</span>
                        </div>
                      )}
                    </>
                  )}
          ...
          <Button
            ...
            icon="Gift"
            onClick={handleClaim}
            disabled={dailyReward.claimedToday}
          >
            {dailyReward.claimedToday ? 'Come Back Tomorrow!' : `Claim Day ${claimableDay} Reward`}
          </Button>

          {dailyReward.claimedToday && (
            <p className="text-xs text-white/40 text-center">You've claimed today's reward. Keep your streak going!</p>
          )}
        </div>
      </div>
    </div>
