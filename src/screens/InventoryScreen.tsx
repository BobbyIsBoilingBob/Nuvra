...
const rarityColors: Record<string, string> = { common: 'border-surface-300 bg-surface-50', rare: 'border-brand-300 bg-brand-50', epic: 'border-accent-300 bg-accent-50', legendary: 'border-error-300 bg-error-50' }
    ...
    <>
      <ScreenShell title="Inventory" subtitle="Your items and treasures">
        {mockItems.length === 0 ? <EmptyState icon={<Backpack size={32} />} title="Empty inventory" message="Complete adventures to find items" actionLabel="Explore" onAction={() => onNavigate('generator')} /> : (
          <div className="grid grid-cols-2 gap-3">{mockItems.map((item, i) => (
            <div key={item.id} className={'rounded-2xl border p-4 stagger ' + rarityColors[item.rarity]} style={{ animationDelay: i * 40 + 'ms' }}>
              <div className="flex items-start justify-between mb-2"><div className="w-12 h-12 rounded-xl bg-white border border-surface-200 flex items-center justify-center shadow-card"><Star size={20} className="text-ink-400" /></div><span className="text-xs font-bold px-2 py-0.5 rounded-full bg-white border border-surface-200">{item.quantity}x</span></div>
              <p className="text-sm font-bold text-ink-900">{item.name}</p><p className="text-xs text-ink-400 mt-0.5 leading-tight">{item.description}</p><p className={'text-xs font-bold uppercase mt-2 ' + rarityText[item.rarity]}>{item.rarity}</p>
            </div>
          ))}</div>
        )}
      </ScreenShell>
