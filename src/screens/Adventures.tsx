      ...
      <div className="relative z-10">
        ...
        <div className="px-4 max-w-md mx-auto flex flex-col gap-4">
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
            <button onClick={() => setTypeFilter(null)} className={`px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all ${!typeFilter?'bg-gradient-to-r from-nova-400 to-cyan-400 text-ink-950':'glass text-white/60'}`}>All Types</button>
            {ADVENTURE_TYPES.map(t => (
              <button key={t.id} onClick={() => setTypeFilter(t.id)} className={`px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all flex items-center gap-1 ${typeFilter===t.id?'bg-gradient-to-r from-nova-400 to-cyan-400 text-ink-950':'glass text-white/60'}`}>
                {t.emoji} {t.label}
              </button>
            ))}
          </div>
          ...
          {/* Theme filter */}
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
            <button onClick={() => setThemeFilter(null)} className={`px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all ${!themeFilter?'bg-gradient-to-r from-plasma-400 to-nova-500 text-ink-950':'glass text-white/60'}`}>All Themes</button>
            {ADVENTURE_THEMES.map(t => (
              <button key={t.id} onClick={() => setThemeFilter(t.id)} className={`px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all flex items-center gap-1 ${themeFilter===t.id?'bg-gradient-to-r from-plasma-400 to-nova-500 text-ink-950':'glass text-white/60'}`}>{t.emoji} {t.name}</button>
            ))}
          </div>
            ...
            {filtered.map(a => (
              <GlassCard key={a.id} className="p-4 cursor-pointer hover:bg-white/[0.1] transition-all active:scale-[0.98]" onClick={() => handleSelect(a)}>
                <div className="flex items-start gap-3">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-plasma-400 to-nova-500 flex items-center justify-center text-3xl flex-shrink-0">{a.emoji}</div>
