...
export default function Inventory() {
  const { ownedItems, equipped, equipItem, setScreen } = useStore();
  ...
  return (
    <Screen>
      <h1 className="font-display text-2xl font-bold text-white mb-4">Inventory</h1>
      <div className="flex gap-2 mb-4 overflow-x-auto no-select pb-1">
        {cats.map((c) => (
          <button key={c} onClick={() => setCategory(c)} className={`px-3 py-1.5 rounded-full text-sm font-medium whitespace-nowrap capitalize ${category === c ? 'bg-zeviqo-500 text-ink-950' : 'bg-ink-700/50 text-ink-400'}`}>{c}</button>
        ))}
      </div>
      {items.length === 0 ? <EmptyState icon={Backpack} title="No items in this category" subtitle="Visit the shop to get items" /> : (
          ...
          {items.map((item) => {
            ...
            return (
              <Card key={item.id} className="p-3" style={{ borderColor: `${RARITY_COLORS[item.rarity]}33` }}>
                ...
                <Button size="sm" variant={isEquipped ? 'secondary' : 'primary'} className="w-full mt-2" onClick={() => equipItem(category, item.id)}>{isEquipped ? <><Check size={14} className="inline" /> Equipped</> : 'Equip'}</Button>
              </Card>
            );
          })}
        </div>
      )}
      <Button variant="ghost" className="w-full mt-4" onClick={() => setScreen('shop')}>Go to Shop</Button>
    </Screen>
  );
}
