...
export function Challenges(): React.ReactElement {
  const { recordChallengeComplete, setScreen } = useStore();
          ...
          {CHALLENGES.map((c) => (
            <GlassCard key={c.id} className="p-4">
              <div className="flex items-start gap-3">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-ember-400 to-gold-500 flex items-center justify-center flex-shrink-0">
                  <Icon name={c.icon} size={22} className="text-ink-950" />
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <Pill icon="Zap" accent="text-nova-300 border-nova-500/30">+{c.xpReward} XP</Pill>
                    <Pill icon="Coins" accent="text-gold-300 border-gold-500/30">+{c.coinReward}</Pill>
                  </div>
                </div>
              </div>
              <Button variant="secondary" size="sm" fullWidth className="mt-3" icon="Check" onClick={() => handleComplete(c.id, c.xpReward, c.coinReward)}>
                Mark Complete
              </Button>
            </GlassCard>
          ))}
