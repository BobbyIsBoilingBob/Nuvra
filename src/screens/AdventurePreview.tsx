...
import { Map, Clock, Zap, Coins, Play, ChevronRight } from 'lucide-react';
...
export default function AdventurePreview() {
  ...
  return (
    <Screen>
      <button onClick={() => setScreen('ai-generator')} className="text-ink-400 text-sm mb-4 flex items-center gap-1"><ChevronRight size={16} className="rotate-180" /> Back</button>
      ...
      <Button size="lg" className="w-full flex items-center justify-center gap-2" onClick={() => setScreen('adventure-map')}><Play size={20} /> Start Adventure</Button>
    </Screen>
  );
}
