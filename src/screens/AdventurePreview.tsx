  ...
  return (
    <Screen>
      ...
      <Card className="p-4 mb-4" style={{ borderColor: `${adventure.color}33` }}>
        ...
        <div className="flex gap-2 mb-3"><Badge color={adventure.color}>{adventure.difficulty}</Badge><Badge color="#fbbf24"><Zap size={10} className="inline" /> {adventure.totalXp} XP</Badge></div>
      ...
      <div className="space-y-2 mb-4">
        {adventure.quests.map((q, i) => { const done = questProgress[q.id]?.completed; return (
          <Card key={q.id} className="p-3 flex items-center gap-3">{done ? <CheckCircle2 size={20} color="#22c55e" /> : <Circle size={20} color="#64748b" />}<div className="flex-1"><p className="text-white font-semibold text-sm">{i + 1}. {q.title}</p><p className="text-ink-400 text-xs">{q.description}</p></div><Badge color="#fbbf24"><Zap size={10} className="inline" /> {q.xp}</Badge></Card>
        ); })}
      </div>
