...
export function Quests(): React.ReactElement {
  ...
  const handleClaim = (q: Quest) => {
    ...
    setRewardData([
      ...(q.xpReward > 0 ? [{ icon: 'Zap', label: 'XP', amount: q.xpReward, color: 'text-zeviqo-300' }] : []),
        ...
        <div className="px-4 max-w-md mx-auto flex flex-col gap-4">
          <div className="flex gap-2 p-1 glass rounded-2xl">
            {(['daily','weekly'] as const).map(t => (
              <button key={t} onClick={() => setTab(t)} className={`flex-1 py-2.5 rounded-xl text-sm font-bold capitalize transition-all ${tab===t?'bg-gradient-to-r from-plasma-400 to-zeviqo-500 text-ink-950':'text-white/50'}`}>{t} Quests</button>
            ))}
          </div>
              ...
              return (
                <GlassCard key={q.id} className={`p-4 ${claimed?'opacity-50':''}`}>
                  <div className="flex items-start gap-3">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${isComplete?'bg-gradient-to-br from-zeviqo-400 to-cyan-300':'glass'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {isComplete && !claimed && <Pill icon="Check" accent="text-zeviqo-300 border-zeviqo-500/30">Ready!</Pill>}
                      ...
                      <div className="mt-2">
                        <ProgressBar value={pct} accent={isComplete?'from-zeviqo-400 to-cyan-300':'from-plasma-400 to-zeviqo-500'} height={6} showShimmer={false} />
                        <div className="flex items-center justify-between mt-1">
                          ...
                          <div className="flex items-center gap-1.5">
                            {q.xpReward > 0 && <span className="text-[10px] text-zeviqo-300 font-bold">+{q.xpReward} XP</span>}
