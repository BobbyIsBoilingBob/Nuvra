...
import { Icon, GlassCard, Pill, Button, ProgressBar, EmptyState, RewardPopup } from '../components/ui';
          ...
          <div className="flex gap-2 p-1 glass rounded-2xl">
            {(['daily', 'weekly'] as const).map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
                className={`flex-1 py-2.5 rounded-xl text-sm font-bold capitalize transition-all ${tab === t ? 'bg-gradient-to-r from-plasma-400 to-nova-500 text-ink-950' : 'text-white/50'}`}
              >
                {t} Quests
              </button>
            ))}
            ...
            {quests.map((q) => {
              ...
              return (
                <GlassCard key={q.id} className={`p-4 ${claimed ? 'opacity-50' : ''}`}>
                  ...
                  {canClaim && (
                    <Button variant="primary" size="sm" fullWidth className="mt-3" icon="Gift" onClick={() => handleClaim(q)}>
                      Claim Reward
                    </Button>
                  )}
                </GlassCard>
