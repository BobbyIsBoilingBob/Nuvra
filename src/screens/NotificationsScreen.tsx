...
export default function NotificationsScreen({ onBack }: Props) {
  ...
  return (
    <ScreenShell title="Notifications" icon={<Bell size={18} className="text-brand-400" />} onBack={onBack}
      headerRight={
        items.some(n => !n.read) ? (
          <button onClick={handleMarkAll} className="text-brand-400 hover:text-brand-300 transition active:scale-95">
            <CheckCheck size={18} />
          </button>
        ) : null
      }
        ...
        <div className="space-y-2">
          {items.map(n => (
            <button key={n.id} onClick={() => !n.read && handleMarkRead(n.id)}
              className={`w-full text-left rounded-xl p-3 border transition active:scale-[0.98] ${n.read ? 'bg-ink-900 border-ink-800' : 'bg-brand-500/10 border-brand-500/30'}`}>
              <div className="flex items-start gap-3">
                <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${n.read ? 'bg-ink-800' : 'bg-brand-500/20'}`}>
                  <Bell size={16} className={n.read ? 'text-ink-500' : 'text-brand-400'} />
                </div>
