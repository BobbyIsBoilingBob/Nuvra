...
export default function NotificationsScreen({ onNavigate }: Props) {
  ...
  const handleReadAll = async () => {
    for (const n of notifications.filter(n => !n.read)) await markNotificationRead(n.id)
    setNotifications(prev => prev.map(n => ({ ...n, read: true })))
    push('success', 'All notifications read')
  }
              ...
              return (
                <button key={n.id} onClick={() => handleRead(n.id)} className={`w-full text-left rounded-xl p-3.5 flex items-start gap-3 border shadow-card stagger transition ${n.read ? 'bg-white border-surface-200' : 'bg-brand-50/50 border-brand-200'}`} style={{ animationDelay: `${i * 40}ms` }}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2"><p className="text-sm font-bold text-ink-900">{n.title}</p>{!n.read && <div className="w-2 h-2 rounded-full bg-brand-500 flex-shrink-0" />}</div>
                    <p className="text-xs text-ink-400 mt-0.5">{n.message}</p>
                    <p className="text-xs text-ink-300 mt-1">{new Date(n.created_at).toLocaleDateString('en', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
                  </div>
                </button>
              )
            })}
