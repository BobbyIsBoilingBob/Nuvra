...
export function Onboarding(): React.ReactElement {
  ...
  const handleFinish = () => completeOnboarding(username.trim() || 'Explorer', { emoji: avatar, color });
        ...
        {step === 1 && (
          <div className="flex-1 flex flex-col gap-4 animate-[fade-in_0.3s_ease-out]">
            ...
            <div className="flex gap-3 justify-center">
              {COLORS.map(c => <button key={c} onClick={() => setColor(c)} className={`w-10 h-10 rounded-full transition-all duration-200 ${color===c?'ring-2 ring-white scale-110':'ring-1 ring-white/20'}`} style={{ background: c }} />)}
            </div>
            <div className="flex justify-center mt-4">
              <GlassCard className="px-6 py-4 flex items-center gap-3">
                <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl" style={{ background: color+'30' }}>{avatar}</div>
                <div><div className="text-sm font-bold text-white">{username || 'Your name'}</div><div className="text-xs text-white/40">Level 1 · Wanderer</div></div>
              </GlassCard>
            </div>
