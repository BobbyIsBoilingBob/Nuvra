    ...
    return (
      <Screen className="flex items-center justify-center">
        <Card className="p-6 text-center">
          ...
          <h2 className="font-display text-xl font-bold text-white mb-2">You're all set!</h2>
          <p className="text-ink-400 text-sm mb-4">Ready to start your adventure?</p>
          <Button onClick={finish} className="w-full">Start Exploring</Button>
        </Card>
      </Screen>
    );
  }
  ...
  return (
    <Screen className="flex flex-col justify-center">
      <div className="flex justify-center gap-1.5 mb-8">
        {steps.map((_, i) => (
          <div key={i} className={`h-1.5 rounded-full transition-all ${i <= step ? 'bg-zeviqo-500 w-8' : 'bg-ink-700 w-4'}`} />
        ))}
      </div>
      <Card className="p-6 text-center">
        ...
        <p className="text-ink-400 text-sm mb-6">{s.desc}</p>
        <div className="flex gap-2">
          {step > 0 && <Button variant="ghost" onClick={() => setStep(step - 1)}>Back</Button>}
          <Button className="flex-1 flex items-center justify-center gap-1" onClick={() => setStep(step + 1)}>
