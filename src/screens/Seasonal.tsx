...
export default function Seasonal() {
    ...
    return (
      <Screen>
        ...
        <Card className="p-6 text-center">
          <Calendar size={32} color="#fbbf24" className="mx-auto mb-3" />
          <h2 className="font-display text-lg font-bold text-white mb-2">Sign in to shop seasonal items</h2>
  ...
  return (
    <Screen>
      ...
      {SEASONAL_ITEMS.length === 0 ? <EmptyState icon={Calendar} title="No seasonal items" subtitle="Check back during special events" /> : (
          ...
          {SEASONAL_ITEMS.map((item) => {
            ...
            return (
              <Card key={item.id} className="p-3" style={{ borderColor: `${RARITY_COLORS[item.rarity]}33` }}>
                ...
                {owned ? (
                  <Button size="sm" variant="secondary" className="w-full mt-2" onClick={() => equipItem(item.category, item.id)}><Check size={14} className="inline" /> Equip</Button>
                ) : (
                  <Button size="sm" variant={item.currency === 'gems' ? 'gold' : 'primary'} className="w-full mt-2" disabled={!canAfford} onClick={async () => { if (!profile) return; const success = buyItem(item.id); if (success) await updateProfile(item.currency === 'coins' ? { coins: profile.coins - item.price } : { gems: profile.gems - item.price }); }}>
