...
export default function AdventureMap() {
  ...
  const toggle = () => {
    if (geo.tracking) { geo.stop(); }
    else { geo.start(); setStartTime(Date.now() - elapsed * 1000); }
  };

  if (!adventure) {
    return <Screen><EmptyState icon={MapPin} title="No adventure selected" /><Button onClick={() => setScreen('adventures')} className="mt-4">Browse</Button></Screen>;
  }
  ...
  const updateDistanceQuests = () => {
    for (const q of adventure.quests) {
      if (q.type === 'distance' && q.target) {
        ...
        if (geo.distance > currentProg) {
          updateQuest(q.id, geo.distance, geo.distance >= q.target);
        }
      }
    }
  };
  ...
  useEffect(() => {
    if (geo.tracking) updateDistanceQuests();
  }, [geo.distance, geo.tracking]);
  ...
  const finish = () => {
    ...
    if (alreadyClaimed) {
      setFinishError('You have already claimed the rewards for this adventure.');
      return;
    }

    if (isGuest || !profile) {
      setFinishError('Sign in to save your progress and earn rewards.');
      return;
    }
    ...
    if (!allQuestsComplete()) {
      const incomplete = adventure.quests.filter((q) => !questProgress[q.id]?.completed);
      setFinishError(`Cannot finish: ${incomplete.length} quest(s) incomplete. ${incomplete.map((q) => q.title).join(', ')}.`);
      return;
    }

    if (!distanceMet) {
      setFinishError(`Cannot finish: you need to walk at least ${formatDistance(minDistance)}. Current: ${formatDistance(geo.distance)}.`);
      return;
    }

    if (!routeMet) {
      setFinishError('Cannot finish: no route data. Start tracking your walk first.');
      return;
    }

    for (const q of adventure.quests) {
      if (!questProgress[q.id]?.completed) {
        updateQuest(q.id, q.target ?? 100, true);
      }
    }
    ...
    addHistory({
      ...
      duration: elapsed,
      xp: adventure.totalXp,
      rating: null,
      completedAt: new Date().toISOString(),
    });

    updateProfile({
      xp: profile.xp + adventure.totalXp,
      coins: profile.coins + Math.floor(adventure.totalXp / 2),
      distance_walked: profile.distance_walked + geo.distance,
    ...
    <Screen>
      ...
      <div className="h-80 mb-4 rounded-2xl overflow-hidden">
        {geo.position ? (
          ...
          <div className="h-full flex items-center justify-center bg-ink-800/60 rounded-2xl">
            <p className="text-ink-400">Waiting for GPS... Press Start to begin.</p>
          </div>
        )}
      </div>
            ...
            return (
              <div key={q.id} className="flex items-center gap-2">
                ...
                <div className="flex-1">
                  ...
                  {q.target && q.type === 'distance' && (
                    <div className="mt-1"><ProgressBar value={prog?.progress ?? geo.distance} max={q.target} color={qDone ? '#22c55e' : '#3b82f6'} /></div>
                  )}
                </div>
