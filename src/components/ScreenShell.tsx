...
interface Props {
  title: string; subtitle?: string; icon?: ReactNode; onBack?: () => void
  children: ReactNode; actions?: Action[]
}
    ...
    <div className="min-h-screen bg-surface-0 animate-slide-up">
      <header className="sticky top-0 z-30 glass safe-top">
        <div className="px-4 py-3 flex items-center gap-3 max-w-md mx-auto">
          {onBack && (
            <button onClick={onBack} className="flex items-center justify-center w-9 h-9 rounded-xl bg-white border border-surface-300 text-ink-700 hover:bg-surface-50 btn-press" aria-label="Go back">
              <ArrowLeft size={18} />
            </button>
          )}
          ...
          {actions?.map((a, i) => (
            <button key={i} onClick={a.onClick} aria-label={a.label} className="flex items-center justify-center w-9 h-9 rounded-xl bg-white border border-surface-300 text-ink-700 hover:bg-surface-50 btn-press">
              {a.icon}
            </button>
          ))}
        </div>
