...
function makeIcon(emoji: string, color: string, size = 32): L.DivIcon {
  return L.divIcon({
    className: '',
    html: `<div class="z-marker" style="width:${size}px;height:${size}px;background:${color};font-size:${size * 0.5}px;">${emoji}</div>`,
    iconSize: [size, size],
...
function makePlayerIcon(): L.DivIcon {
  return L.divIcon({
    className: '',
    html: `<div class="z-marker z-pulse" style="width:18px;height:18px;background:#43c79b;font-size:1px;">&nbsp;</div>`,
    iconSize: [18, 18],
    iconAnchor: [9, 9],
  })
}
...
// Component that fits the map to the route bounds
  ...
  return (
    ...
    >
      ...
      <Polyline
        ...
        pathOptions={{ color: '#43c79b', weight: 4, opacity: 0.8 }}
      />

      {/* Checkpoint markers */}
      {route.checkpoints.map((cp) => {
        const isStart = cp.isStart
        ...
        return (
          <Marker
            ...
            icon={makeIcon(emoji, color, size)}
            eventHandlers={{
              click: () => onCheckpointClick?.(cp.index),
            }}
          />
        )
      })}
      ...
      {/* Fallback player circle if no marker icon */}
      {playerLocation && (
        <CircleMarker
          center={[playerLocation.lat, playerLocation.lng]}
          radius={6}
          pathOptions={{ color: '#43c79b', fillColor: '#43c79b', fillOpacity: 0.8 }}
        />
      )}
