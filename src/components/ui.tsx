...
export function SectionTitle({ children, icon, accent = 'text-white' }: SectionTitleProps): React.ReactElement {
  return (
    <div className="flex items-center gap-2">
      {icon && <Icon name={icon} size={16} className={accent} />}
      <h3 className={`text-xs font-black uppercase tracking-wider ${accent}`}>{children}</h3>
    </div>
  );
}
...
export function Spinner({ size = 24, className = '' }: SpinnerProps): React.ReactElement {
  return (
    <div
      className={`animate-spin rounded-full border-2 border-white/10 border-t-nova-400 ${className}`}
...
export function LoadingScreen(): React.ReactElement {
  ...
  useEffect(() => {
    const id = setInterval(() => setTip((t) => (t + 1) % LOADING_TIPS.length), 3000);
    return () => clearInterval(id);
  }, []);
  return (
    <div className="flex flex-col items-center justify-center min-h-screen gap-4">
      <div className="relative">
        <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-nova-400 to-plasma-500 flex items-center justify-center text-3xl animate-pulse">
          🧭
        </div>
      </div>
      <Spinner size={32} />
      <p className="text-xs text-white/40 max-w-xs text-center animate-[fade-in_0.3s_ease-out]">
        {LOADING_TIPS[tip]}
      </p>
    </div>
  );
...
// ------------------------------------------------------------
// 10. RewardPopup — animated reward notification
// ------------------------------------------------------------
export interface RewardPopupProps {
  rewards: Array<{ icon: string; label: string; amount: number; color: string }>;
  visible: boolean;
  onClose: () => void;
}
...
export function LevelBadge({ level, size = 'md' }: { level: number; size?: 'sm' | 'md' | 'lg' }): React.ReactElement {
  ...
  return (
    <div className={`${sizes[size]} rounded-xl bg-gradient-to-br from-nova-400 to-plasma-500 flex items-center justify-center font-black text-ink-950 no-select`}>
      {level}
    </div>
  );
}
...
// ------------------------------------------------------------
// 12. XpBar — shows current level progress
// ------------------------------------------------------------
...
export function Modal({ visible, onClose, title, children }: ModalProps): React.ReactElement | null {
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-center justify-center" onClick={onClose}>
      ...
      <div
        className="relative glass-strong rounded-2xl p-5 max-w-sm mx-4 w-full animate-[scale-in_0.2s_ease-out]"
        onClick={(e) => e.stopPropagation()}
      >
        {title && (
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-black text-white">{title}</h3>
            <button onClick={onClose} className="w-8 h-8 rounded-lg glass flex items-center justify-center text-white/60">
              <Icon name="X" size={16} />
            </button>
          </div>
        )}
