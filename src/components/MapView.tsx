...
export default function MapView({
  center = DEFAULT_CENTER, route = [], markers = [], showUserLocation = false,
  zoom = 15, className = '', interactive = true,
}: MapViewProps) {
  ...
  useEffect(() => {
    ...
    if (container.offsetHeight === 0) container.style.height = '100%';

    const map = L.map(container, {
      center: [center.lat, center.lng], zoom,
      zoomControl: interactive, dragging: interactive, scrollWheelZoom: interactive,
      doubleClickZoom: interactive, touchZoom: interactive, attributionControl: false,
    });
    mapRef.current = map;
    ...
    // CRITICAL: invalidateSize after the lazy-loaded container has rendered.
    // Leaflet computes tile positions against the container's box; if the box
    // has zero height at init time (common with lazy-loaded components),
    // tiles never appear. The setTimeout lets the browser lay out the
    // container with its real dimensions before we ask Leaflet to recompute.
    const invalidateTimer = setTimeout(() => { map.invalidateSize(); }, 200);

    // Also watch for container resize (e.g. keyboard open, orientation change)
  ...
  useEffect(() => {
    if (mapRef.current) mapRef.current.setView([center.lat, center.lng], zoom, { animate: true });
  ...
  useEffect(() => {
    ...
    if (route.length > 0) {
      ...
      if (latlngs[0]) L.marker(latlngs[0], { icon: createEmojiMarker('🚩', '#22c55e') }).addTo(map);
      if (latlngs[latlngs.length - 1]) L.marker(latlngs[latlngs.length - 1], { icon: createEmojiMarker('🏁', '#ef4444') }).addTo(map);
    }

    for (const m of markers) {
      const marker = L.marker([m.lat, m.lng], { icon: createEmojiMarker(m.emoji ?? '📍', m.color ?? '#00c4ff') }).addTo(map);
