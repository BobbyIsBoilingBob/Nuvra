...
interface Props {
  challenge: Challenge; sensorAvail: SensorAvailability
    ...
    if (challenge.category === 'fitness' && sensorAvail.accelerometer) {
      motionHandler.current = (e: DeviceMotionEvent) => {
        ...
        if (acc) {
          ...
          if (mag > 15) setStepCount(s => s + 1)
        }
      }
      window.addEventListener('devicemotion', motionHandler.current)
      return () => { if (motionHandler.current) window.removeEventListener('devicemotion', motionHandler.current) }
    }
  ...
  if (showResult) {
    return (
      <div className="flex flex-col items-center justify-center py-8 animate-bounce-in">
        <div className="w-16 h-16 rounded-full bg-gradient-to-br from-success-500 to-success-600 flex items-center justify-center mb-3 shadow-glow-brand">
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center gap-2 mb-3">
        ...
        <div>
          <p className="text-sm font-bold text-ink-900">{challenge.title}</p>
          <p className="text-xs text-ink-400">{challenge.description}</p>
        </div>
      </div>
      ...
      {challenge.category === 'trivia' && challenge.options && (
        <div className="space-y-2">
          ...
          {challenge.options.map((opt, i) => (
            <button key={i} onClick={() => setSelectedAnswer(i)} disabled={done}
              className={`w-full text-left px-4 py-3 rounded-xl border-2 text-sm font-medium transition btn-press ${selectedAnswer === i ? 'bg-brand-50 border-brand-500 text-brand-700' : 'bg-white border-surface-300 text-ink-700 hover:border-brand-400'}`}>
              {opt}
            </button>
          ))}
          <button onClick={handleComplete} disabled={selectedAnswer === null}
            className="btn-primary mt-2 disabled:opacity-40 disabled:cursor-not-allowed">Submit Answer</button>
        </div>
      )}
      ...
      {challenge.category === 'photo' && (
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-center w-full h-32 rounded-xl border-2 border-dashed border-surface-400 bg-surface-50">
      ...
      {challenge.category === 'fitness' && (
        <div className="space-y-3 text-center">
          ...
          <div className="h-3 bg-surface-200 rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-brand-500 to-brand-600 rounded-full transition-all duration-300" style={{ width: `${Math.min((stepCount / (challenge.targetSteps ?? 1)) * 100, 100)}%` }} />
          </div>
          {!sensorAvail.accelerometer && <p className="text-xs text-warning-600">Sensor not available - tap complete</p>}
          <button onClick={handleComplete} disabled={stepCount < (challenge.targetSteps ?? 0) && sensorAvail.accelerometer}
            className="btn-primary disabled:opacity-40 disabled:cursor-not-allowed">Complete Challenge</button>
        </div>
      )}
      ...
      {challenge.category === 'compass' && (
        <div className="space-y-3 text-center">
          ...
          <div className="relative w-24 h-24 mx-auto">
            ...
            <div className="absolute inset-0 flex items-center justify-center" style={{ transform: `rotate(${compassHeading}deg)` }}>
      ...
      {challenge.category === 'riddle' && (
        <div className="space-y-3">
          ...
          <button onClick={handleComplete} disabled={!riddleInput.trim()}
            className="btn-primary disabled:opacity-40 disabled:cursor-not-allowed">Submit Answer</button>
        </div>
      )}
...
function CategoryIcon({ category }: { category: string }) {
  const icons: Record<string, typeof Check> = {
    trivia: Brain, photo: Camera, fitness: Footprints, compass: Navigation,
    riddle: HelpCircle, speed: Zap, exploration: Navigation, puzzle: Brain,
  }
