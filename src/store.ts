...
export interface AdventureHistoryEntry {
  ...
  completedAt: number; players: string[]; isFavorite: boolean;
}
...
interface StoreState {
  ...
  recordAdventureComplete: (adventure: Adventure, xp: number, coins: number, gems: number, isMultiplayer: boolean, players: string[], distance: number, time: number, treasures: number, maxCombo: number) => void;
...
const defaultProfile: Profile = {
  ...
  username: 'Explorer', avatar: { emoji: '🧭', color: '#33ffd6' },
  xp: 0, coins: 0, gems: 0,
...
export const useStore = create<StoreState>()(
  persist(
    (set, get) => ({
      ...
      recordAdventureComplete: (adventure, xp, coins, gems, isMultiplayer, players, distance, time, treasures, maxCombo) => {
        const state = get();
        const historyEntry: AdventureHistoryEntry = {
          ...
          treasuresFound: treasures, maxCombo, completedAt: Date.now(),
      ...
      updateQuestProgress: (_category, _amount) => {
        // Quest progress is derived from stats in the UI
      },

      claimQuest: (questId) => {
        const state = get();
        set({ questProgress: state.questProgress.map(q => q.questId === questId ? { ...q, claimed: true } : q) });
      },
      ...
      checkAchievements: () => {
        ...
        for (const [id, value] of Object.entries(checks)) {
          ...
          else if (value >= 1) { newlyUnlocked.push(id); updated.push({ achievementId: id, unlocked: true, dateUnlocked: todayStr(), progress: 1 }); }
      ...
      checkDailyReward: () => {
        ...
        if (state.dailyReward.lastClaimDate !== today) set({ dailyReward: { ...state.dailyReward, claimedToday: false } });
      },
      ...
      repeatAdventure: (historyId) => {
        ...
        // Create a synthetic adventure from history
        return {
